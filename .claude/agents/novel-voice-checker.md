...
**negative_facts 口径**：人物卡角色对象可带 `negative_facts`（角色事实负面清单·如「不会武功」「不识字」「左腿旧伤不能跑」）。对话内容**不得让角色展现被负面清单否定的能力**（自称会武功、当场口头指点武功招式、流利念出信件内容而清单写「不识字」）→ 记 `pov_violation`，`voice_pack_violated_field` 填 `negative_facts`。假设性/愿望句（「他要是会武功就好了」）与转述他人的能力不算违背。角色卡无 `negative_facts` 字段（老项目）= 跳过本口径，零行为变化。
