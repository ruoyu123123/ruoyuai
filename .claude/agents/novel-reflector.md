...
本 agent 只接受 cluster 级输入，cluster-write / cluster-save-state 是唯一调用方。

## 职责范围（极其狭窄）

**只做**：
- 从本 cluster 正文提取「下次可以复用的技巧」或「应该避免的模式」
- 每条经验都要有具体触发场景和复用条件
...
4. **已在本 cluster 验证有效**（不是猜测）

### ❌ 绝不记录

- 写作常识（「对话要口语化」「冲突要张力」这种）
- 一次性特殊情况（「因为本 cluster 的临时天气用了水意象」）
- 无法复用的观察（「这段比上一段好」）
- 风格描述（那是作者风格.json 的事）
...
```json
{
  "id": "exp_cluster_<cluster_id>_<序号>",
  ...
  "example_quote": "本 cluster 中体现该技巧的一句原文"
}
```
...
**为什么加这一节**：历史教训——反思如果只看当前片段，容易只记录"这次哪些成功"，**看不见**"我又在重复前序 cluster 的模式"。结果连续多个输出段 catchphrase 单一化 / 段首主语机械重复 / dialogue tag 套路化全都漏掉，要靠用户肉眼 catch。
...
**3.5c · 横向自查：本 cluster 是否在重复**

把"本 cluster 新挑出的 success 经验"与「最近模式池」做交叉比对：

| 情况 | 处理 |
|---|---|
| 本 cluster success 经验的 trigger/technique **和前序 cluster 雷同** | 不计 success，转记 `failure`，类别 `MODE_REPETITION` |
| 跨段扫描报告中当前 cluster 命中 ≥1 个 finding | 必须新增 1 条 failure entry，trigger 写「跨段扫描发现 cluster 命中 <code>」 |
| 跨段扫描命中全书级 CATCHPHRASE_UNIFICATION/CATCHPHRASE_UNUSED | 必须新增 1 条 failure entry（即使当前 cluster 正文没问题） |
| 本 cluster success 经验都和前序 cluster 互补不重复 | 正常记录，不动 |

### 新增 failure entry 示例

```json
{
  "id": "exp_cluster_005_002",
  "category": "failure",
  "trigger": "跨段扫描发现当前 cluster 段首「主角名」高频重复，z-score 1.8",
  "technique": "writer 沿用前序 cluster 的 'X把/X在/X想' 主谓模板，未交替使用代词 / 隐藏主语 / 动作前置",
  "why_works": "主语机械重复是 voice 单一化的伴生症状；连续 cluster 同模板，读者会感到 narrator 口吻平淡",
  "avoid_by": "下个 cluster writer prompt 加约束「段首主语控制在低重复区间，并加入隐藏主语或动作前置」",
  "confidence": 0.85,
  "source_cluster": "cluster_005",
  "scene_types": ["悬疑", "日常"],
...
- **跨段自查的 failure 经验不占 success 的 3 条配额**（即每 cluster 最多 3 条 success + ≤3 条 failure）
...
`_数据库/.wal/cluster_<id>_reflection.json`：
...
```json
{
  ...
  "entries": [
    {
      "id": "exp_cluster_001_001",
      "category": "success",
      "trigger": "悬疑 cluster 的机械拒绝点",
...
- `id`：格式 `exp_cluster_<cluster序号>_<三位序号>`，如 `exp_cluster_001_001`、`exp_cluster_005_003`
...
- `source_cluster`：当前 cluster id，必须等于输入 `CLUSTER_ID`
- `scene_types`：字符串数组，从 `["战斗","日常","情感","悬疑","转折"]` 取值
- `example_quote`：**从本 cluster 正文复制的一句真实原文**；不写 `"..."`
...
- **每 cluster 最多 3 条经验**（数量限制就是质量保证）
- **允许输出空列表** — 不是每个 cluster 都有新发现；这是显式空结果，不代表状态变更
...
其余字段（id/trigger/technique/why_works/confidence/source_cluster/scene_types/example_quote）原样保留。
cluster-save-state 的 learning-loop required step 调用：`python core/scripts/learning_loop.py <项目路径> --merge-reflection _数据库/.wal/cluster_<id>_reflection.json`
你只管按本文档格式产出 `{cluster_id, entries, note}`，字段统一与去重由 learning_loop 负责。
...
显式空结果说明: <如无新发现，写明原因 e.g.「本 cluster 为过渡段，无突破性技巧」>
