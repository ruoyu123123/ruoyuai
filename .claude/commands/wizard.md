...
### 组 1：项目基础（必问 · v23.11 强化）

> ⚠️ **本组结果会被 /outline 第 1.5 步消费反推 event 池密度**，缺失会导致大纲只支撑总章数的 1/4（历史 ch80 翻车，见 lesson `feedback_outline_chapter_density_design`）。

- **`target_chapter_count` 总章节数**（**required**，写入用户偏好.json）：
  - 短篇：30-60 章
  - 中篇：60-150 章
  - 长篇：150-300 章
  - 网文超长：300-800 章（**默认 500**）
  - 史诗：800+ 章
- **`volume_count` 卷数**（默认 6）
- **`rhythm_profile` 节奏档位**（默认「混合」 · 影响 event/卷 + 章/event）：
  - 紧凑：16-20 event/卷 × 4-5 章/event（短中篇）
  - 标准：10-12 event/卷 × 6-8 章/event（主流网文）
  - 厚重：6-8 event/卷 × 10-14 章/event（史诗）
  - **混合（推荐）**：8-10 event/卷 × 4-12 章（重轻分级）
- **`filler_ratio` 日常/支线占比**（默认 0.2，范围 0.1-0.3）
- **每章字数目标**：
  - 网文：1800-3000 / 短篇正文：2500-4000 / 长篇：3000-5000
- **DCAS 双章模式**：默认 true（推荐）
- **写作模式**：完整/轻量

**自动校验**：T × (1-F) / (V × E) 算出的 A 必须在 4-12 范围；不在 → 提示用户冲突点让用户改 E/F/V。
