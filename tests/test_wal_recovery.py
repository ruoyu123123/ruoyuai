# -*- coding: utf-8 -*-
"""wal_recovery WAL 断点恢复回归网（第四轮 Workflow #6·2026-06-17）。

WAL 恢复逃生舱·commit 877eb87 修过严重 bug：plan_tracker 用 completed_at/aborted_at 时间戳
（顶层无 status 字段）→ 原 p.get("status") 恒 None → 所有 plan 误判中断 exit1 + 错误续跑指令。

覆盖：
  · _parse_plan_list_output（纯正则·真实）
  · _filter_plans_by_cluster（cluster 过滤判据 1·真实）
  · 完成/中断分类（复现 main L207-211 _done/_aborted 闭包·status-None 回归核心·闭包难直接 import）
...
def test_parse_plan_list_basic():
    # 真实 plan_tracker list 输出格式（plan_tracker.py:1128 `[{flag:6}]` 左对齐·双空格分隔）
...
def test_filter_by_cluster_key_match():
    """plan.key 归一化 == 目标 cluster（"001"→"cluster_001"·SC-5 不机械拼接）。"""
...
# ============ 完成/中断分类（复现 main L207-211 闭包·status-None 回归核心）============
# 闭包在 main() 内不可直接 import·此处复现其逻辑测「分类原则」（commit 877eb87 修复）。
def _done(p):
...
def test_classify_done_by_completed_at():
    """🔴 回归 877eb87：completed_at 时间戳判 done（顶层无 status·原 p.get(status) 恒 None 误判全中断）。"""
