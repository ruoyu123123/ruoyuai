...
# ═══════════════════ 内容语义 embedding 接线（2026-07-04 · content_backend_available 门控 + 字面 fallback）═══════════════════
...
def test_drift_score_semantic_catches_zero_overlap_synonym(monkeypatch):
    """内容后端就绪：字面零重叠的同义改写（「殊死搏杀」vs「决一死战」无共享 2-gram）应被余弦
    相似度识别为低漂移——字面 2-gram Jaccard 会误判成完全偏离（drift 必为 1.0）。"""
    import embedding_store
    # 前置断言：确认字面法确实判为完全偏离（两词各 4 字·3 个 2-gram 全不相交）——门控显式关闭
    # （content_backend_available 查真实磁盘·不能靠"不设环境变量"隐式达成关闭）。
    monkeypatch.setattr(embedding_store, "content_backend_available", lambda: False)
...
def test_drift_score_semantic_falls_back_on_embedding_error(monkeypatch):
    """内容后端配置但编码异常 → 回退字面 2-gram Jaccard（不崩·结果与门控关时一致）。"""
    import embedding_store
    monkeypatch.setattr(embedding_store, "content_backend_available", lambda: True)
...
def test_drift_score_default_matches_literal_jaccard_exactly(monkeypatch):
    """🔴 零回归锁：门控关 → _drift_score 与直接手算字面 Jaccard 逐位一致。"""
    import embedding_store
    monkeypatch.setattr(embedding_store, "content_backend_available", lambda: False)
...
def test_high_drift_threshold_used_in_summary():
    """🔴 阈值接线锁：cmd_summary 的 _threshold 字段与 HIGH_DRIFT_THRESHOLD 常量同源
    （2026-07-04 金标准校准 0.53·不是裸写死的另一个数字）。"""
    assert mod.HIGH_DRIFT_THRESHOLD == 0.53
