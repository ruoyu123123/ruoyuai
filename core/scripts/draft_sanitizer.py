# -*- coding: utf-8 -*-
"""draft_sanitizer.py — gen-model 草稿确定性清洗（2026-06-07 根治）

治两类 gen-model 原始输出的机械格式病（cluster_001 实测翻车、用户肉眼逮到）：

1. **成对符号腰斩**：引号“”/方括号【】/书名号《》/圆括号（）/单引号‘’ 被句末标点 +
   换行劈开，右半边掉到下一段（如 `【说明：…实体化身。\\n\\n】`、`“…抢救！\\n\\n”他`）。
   质检长期只查引号没查【】，靠人读逐个逮 → 此处统一确定性合并。

2. **整块逐字复制**：freestyle expand 续写兜底时把前文整块复制一遍（RR_001，
   cluster_001 草稿前 200 段被原样重述，字数虚涨 23770，读者读到一半以为卡带）。
