...
def canary_evaluate(project_root: Path, canary_window: int = 5) -> dict:
    """对最近一次 evolution 做 canary 评估"""
    ...
    if len(chapters) < canary_window:
        return {"status": "insufficient_chapters", "need": canary_window, "have": len(chapters)}

    window_chs = chapters[-canary_window:]
