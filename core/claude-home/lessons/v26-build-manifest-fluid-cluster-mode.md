...
```python
if not self.cluster_blueprint():
    fatal.append(f"cluster_blueprint[{self.ch}] 不存在，大纲未覆盖本章")
```

`cluster_blueprint()` 方法（line 119）：
```python
def cluster_blueprint(self) -> dict | None:
    prog = self.load("进度", {})
    plans = prog.get("cluster_blueprint", [])
    for p in plans:
...
**问题**：cluster mode 下，cluster_002 的具体章数 + 章细节应由 step 6 splitter 切 cluster_draft.txt 时决定。**预先在 cluster_blueprint 锁 ch6-11 全部细节 = chapter mode 思维残留**。

调度器主代理被 fatal 误导，给 cluster_blueprint 加了 ch6-11 6 章占位 —— **违反 CLAUDE.md fluid 涌现哲学**：
...
```python
if not self.cluster_blueprint():
    # v26 cluster mode fluid 化: cluster_blueprint[N] 缺失不阻塞 · writer 从 事件簇.json.clusters[N] 取信息
    warning.append(f"cluster_blueprint[{self.ch}] 不存在 (cluster mode fluid · 信息从 事件簇.json 取)")
```

### Option B（更彻底 · cluster-aware fallback）
`cluster_blueprint()` 加 cluster fallback：
```python
def cluster_blueprint(self) -> dict | None:
    prog = self.load("进度", {})
    plans = prog.get("cluster_blueprint", [])
...
- step 1 build_manifest 在 cluster mode (key 含 cluster_) 下完全不调 cluster_blueprint(N) · 改读 事件簇.json.clusters[N]
- cluster_blueprint 字段只为 chapter mode 保留（v26 已废 chapter mode · 但 cluster_blueprint 数据结构仍被其他脚本读 · 不能简单删）
...
候选 lesson key: `feedback_build_manifest_cluster_mode_fluid_cluster_blueprint_fallback`
