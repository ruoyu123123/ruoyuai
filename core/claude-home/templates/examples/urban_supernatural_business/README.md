...
| `涟漪规则.example.json` | 小势→世界状态涟漪规则（北极星②）——minor_event / fate_event / auto_tick 三类 trigger，fate_event 引用大势卡 ME id，含 `RR_AUTO_TICK` 每章世界推进 |
| `角色行动表.example.json` | PbtA Moves 声明式行动数据——每角色一组 move（trigger + `narrative_template` 框 + consequence + `frequency_per_chapter` 防公式化），`characters{}.moves[]` + `offstage_threads{}` |
...
### Moves 设计要点
- 每角色 4-6 个 move 够用（多了会冲突 `frequency_per_chapter`）
...
模板内容来自一个都市超自然商战题材退役项目的实战运行数据，已脱敏。
