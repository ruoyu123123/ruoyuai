...
| `steps[].expected_outputs` | 本步必须落盘的产物（`plan_tracker step` 时校验存在）；主链 required step 必须配置非空 expected_outputs 或 touch_outputs 代理产物 | |
...
- `<chapter_range_dash>` / `<rhythm>` / `<style_name>` 等：经 `data_flow` 声明从对应
  source_json 字段取值回填。
- `{project_root}` / `{key}` / `{cluster_id}` / `{next_key}`：由 `plan_tracker` 在 create 时替换。`{ch}` 系列旧章号占位符会被硬拒。
